...
def target(trail: optuna.Trial, cfg):
    ...
    max_learning_rate = trail.suggest_float('max_lr', 5, 100, log=True)
...
def main(cfg: DictConfig):
    ...
    study.optimize(target_with_cfg, n_trials=100)
    ...
    plt.savefig(dataset + 'K_' + str(cfg.setup.algorithm.perturb_rate) + '_' + str(cfg.setup.algorithm.if_overlap) + '_parento.jpg')
    fig = plot_param_importances(study, target=lambda t: t.values[0])
    plt.savefig(dataset + 'K_' + str(cfg.setup.algorithm.perturb_rate) + '_' + str(cfg.setup.algorithm.if_overlap) + '_acc_importance.jpg')
    fig = plot_param_importances(study, target=lambda t: t.values[1])
    plt.savefig(dataset + 'K_' + str(cfg.setup.algorithm.perturb_rate) + '_' + str(cfg.setup.algorithm.if_overlap) + '_query_importance.jpg')
    fig = plot_parallel_coordinate(study, target=lambda t: t.values[0])
    plt.savefig(dataset + 'K_' + str(cfg.setup.algorithm.perturb_rate) + '_' + str(cfg.setup.algorithm.if_overlap) + '_acc_cord.jpg')
    fig = plot_parallel_coordinate(study, target=lambda t: t.values[1])
    plt.savefig(dataset + 'K_' + str(cfg.setup.algorithm.perturb_rate) + '_' + str(cfg.setup.algorithm.if_overlap) + '_query_cord.jpg')
